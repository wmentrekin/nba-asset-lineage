...
export function chapterFocus(
  ...
  fallbackBounds: { start: string; end: string },
  chapterLayout?: TimelineChapterLayout | null,
): TimelineFocus {
  void chapter;
  const windowStart = chapterLayout?.window_start ?? fallbackBounds.start;
  const windowEnd = chapterLayout?.window_end ?? fallbackBounds.end;
  return {
    windowStart,
    windowEnd,
    zoom: chapterLayout?.default_zoom ?? 1,
    assetKinds: [...ASSET_KIND_ORDER],
    eventIds: [...(chapterLayout?.highlight_event_ids ?? [])],
    assetIds: [...(chapterLayout?.highlight_asset_ids ?? [])],
    laneGroups: [],
    highlightMode: "focus",
  };
}
  ...
  return (contract.editorial?.story_chapters ?? [])
    ...
    .map((chapter) => {
      ...
      return {
        ...
        windowStart: chapterLayout?.window_start ?? bounds.start,
        windowEnd: chapterLayout?.window_end ?? bounds.end,
        defaultZoom: chapterLayout?.default_zoom ?? null,
        highlightAssetIds: [...(chapterLayout?.highlight_asset_ids ?? [])],
        highlightEventIds: [...(chapterLayout?.highlight_event_ids ?? [])],
