...
describe("timeline utilities", () => {
  ...
  it("keeps editorial chapter title, body, and dates additive instead of sourcing scene windows from chapter_layout", () => {
    ...
    const remappedLayout = cloneJson(layoutContract) as unknown as TimelineGeneratedLayoutContract;
    const focusAssetId = presentationContract.edges[0]?.asset_id;
    const focusEventId = presentationContract.nodes.find((node) => node.event_id)?.event_id;

    expect(focusAssetId).toBeDefined();
    ...
    (
      remappedChapters[0] as TimelineChapterExport & {
        focus_payload?: {
          date_range?: {
            start_date?: string;
            end_date?: string;
      ...
      event_ids: ["legacy-focus-event"],
      asset_ids: ["legacy-focus-asset"],
      default_zoom: 99,
    };
    remappedLayout.chapter_layout[0]!.window_start = "2024-02-01";
    remappedLayout.chapter_layout[0]!.window_end = "2024-02-29";
    remappedLayout.chapter_layout[0]!.highlight_asset_ids = [focusAssetId!];
    remappedLayout.chapter_layout[0]!.highlight_event_ids = [focusEventId!];
    remappedLayout.chapter_layout[0]!.default_zoom = 5;
    ...
    expect(scene.chapters[0]).toMatchObject({
      ...
      defaultZoom: 5,
      highlightAssetIds: [focusAssetId],
      highlightEventIds: [focusEventId],
      minimapAnchorId: "layout_minimap_segment_cd9fbb9b2c49718691e39207",
    });
    expect(scene.chapters[0]?.windowStart).not.toBe("2001-01-01");
    expect(scene.chapters[0]?.highlightAssetIds).not.toEqual(["legacy-focus-asset"]);
  });
